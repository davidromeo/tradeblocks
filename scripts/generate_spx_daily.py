...
def merge_with_existing(new_df, existing_df):
    """Merge new rows into existing DataFrame, preserving intraday-derived fields."""
    intraday_cols = ["High_Time","Low_Time","High_Before_Low","High_In_First_Hour",
                     "Low_In_First_Hour","High_In_Last_Hour","Low_In_Last_Hour",
                     "Reversal_Type","High_Low_Spread","Early_Extreme","Late_Extreme"]
    existing = existing_df.set_index("date")
    new = new_df.set_index("date")
    # Preserve intraday columns from existing where new has NaN
    for col in intraday_cols:
        if col in existing.columns and col in new.columns:
            new[col] = new[col].combine_first(existing[col])
    combined = new.combine_first(existing).reset_index()
    return combined[new.reset_index().columns]
...
if __name__=="__main__":
    ...
    p.add_argument("--merge",action="store_true",
                   help="Merge with existing CSV at --output path, preserving intraday fields")
    a=p.parse_args()

    # Save existing data before build() overwrites it
    existing_df = None
    if a.merge and Path(a.output).exists():
        print(f"Reading existing {a.output} for merge ...")
        existing_df = pd.read_csv(a.output)

    build(a.start, a.output)

    if existing_df is not None:
        print(f"Merging with {len(existing_df)} existing rows ...")
        new_df = pd.read_csv(a.output)
        merged = merge_with_existing(new_df, existing_df)
        merged.to_csv(a.output, index=False, float_format="%.6f")
