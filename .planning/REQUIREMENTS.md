...
- [x] **ENR-04**: Tier 3 enrichment: intraday timing fields written to `market.daily` from `market.intraday` bars (High_Time, Low_Time, Reversal_Type, Opening_Drive_Strength, Intraday_Realized_Vol, etc.) — *implemented in Phase 64-03; blocker resolved in Phase 64-02 via auto-extraction of time from Unix timestamp*
- [x] **ENR-05**: 200+ day lookback for Wilder smoothing warmup, only updating requested date range
- [x] **ENR-06**: Efficient incremental enrichment via `enriched_through` date watermark in `_sync_metadata` with 200-day lookback window for Wilder smoothing warmup — watermark approach re-derives state from lookback each run (no persisted Wilder state)
