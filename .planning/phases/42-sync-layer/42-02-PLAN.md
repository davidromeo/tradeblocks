...
must_haves:
  truths:
    ...
    - "Server startup does NOT trigger sync (lazy sync)"
  artifacts:
...
3. **Copy CSV parsing helpers locally** (from block-loader.ts - they're ~50 lines, self-contained, avoids circular import risk):
   ```typescript
   /**
...
4. **`syncBlockInternal(conn, blockId, blockPath): Promise<BlockSyncResult>`**
   ...
   - If hash differs or no metadata exists:
     a. `await conn.run("BEGIN TRANSACTION")`
     b. `await conn.run("DELETE FROM trades.trade_data WHERE block_id = ?", blockId)`
     c. Parse CSV using `parseCSV()` (the local copy)
     d. Call `insertTradeBatch()` for trades in batches of 500 rows
     e. Hash dailylog and reportinglog if they exist
     f. `upsertSyncMetadata()` with new hashes
     g. `await conn.run("COMMIT")`
     h. Return `{ status: 'synced', tradeCount }`
   - On error: `await conn.run("ROLLBACK")`, return `{ status: 'error', error: message }`

5. **`detectBlockChanges(conn, baseDir): Promise<{ toSync: string[], toDelete: string[] }>`**
...
6. **`cleanupDeletedBlocks(conn, deletedBlockIds): Promise<void>`**
...
7. **Helper: `insertTradeBatch(conn, blockId, trades: Record<string, string>[], startIdx, batchSize): Promise<void>`**
   Build and execute multi-row INSERT using DuckDB parameter binding:
   ```typescript
   async function insertTradeBatch(
     ...
     // Build VALUES placeholders: ($1, $2, $3, ...), ($14, $15, $16, ...), ...
     ...
     for (let rowIdx = 0; rowIdx < batch.length; rowIdx++) {
       ...
       params.push(
         ...
         parseFloat(record["Premium"]) || null,             // premium
         parseInt(record["No. of Contracts"], 10) || 1,     // num_contracts
         parseFloat(record["P/L"]) || 0,                    // pl
         record["Date Closed"] || null,                     // date_closed
         record["Time Closed"] || null,                     // time_closed
         record["Reason For Close"] || null,                // reason_for_close
         parseFloat(record["Margin Req."]) || null,         // margin_req
         parseFloat(record["Opening Commissions + Fees"]) || 0, // opening_commissions
         parseFloat(record["Closing Commissions + Fees"]) || 0  // closing_commissions
       );
     }
   ...
   ```
   **Note:** DuckDB Node API accepts parameters as rest args after the SQL string: `conn.run(sql, param1, param2, ...)`. The `$1, $2, ...` placeholders are positional.
...
5. **Transaction rollback test**: Verify that on parse error mid-sync, no partial trade data exists in DuckDB:
   - Create a malformed CSV (valid header, corrupt data rows)
   - Attempt sync
   - Verify trades.trade_data has NO rows for that block_id (rollback worked)
6. **SYNC-06 (lazy sync)**: Verify server init does NOT call sync:
   - Grep `packages/mcp-server/src/index.ts` for sync function imports/calls
   - Confirm sync functions are NOT called at module load or server init
   - Sync only happens when tools call `syncAllBlocks()` or `syncBlock()`
</verification>
...
- Server startup does NOT trigger sync (lazy initialization verified)
</success_criteria>
