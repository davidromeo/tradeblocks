...
must_haves:
  ...
  key_links:
    ...
    - from: "packages/mcp-server/src/db/connection.ts"
      ...
      via: "ensureMarketDataTables called during init"
...
<task type="auto">
  <name>Task 1: Add market data table schemas and wire to connection init</name>
  <files>
    packages/mcp-server/src/db/schemas.ts
    packages/mcp-server/src/db/connection.ts
  </files>
...
5. **Update `packages/mcp-server/src/db/connection.ts`** to call `ensureMarketDataTables(connection)` after `ensureTradeDataTable(connection)`:
   ```typescript
   import { ensureSyncTables, ensureTradeDataTable, ensureMarketDataTables } from "./schemas.js";

   // ... in getConnection(), after schema creation:
...
- connection.ts imports and calls `ensureMarketDataTables`
  </verify>
