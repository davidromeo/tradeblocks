...
must_haves:
  truths:
    ...
    - "Factual observations include ALL metric comparisons from every signal (no threshold filtering) -- ~19-21 observations as structured data objects with signal, metric, current, comparison, delta, percentChange fields"
...
<task type="auto">
  ...
  **Test cases to write (these will fail initially -- RED phase):**
  ...
  8. **Observations are exhaustive (no threshold filtering)**: ALL metric comparisons from every available signal are included as observations. For 50+ trades with no reporting log, expect observations from rolling metrics (all recentVsHistorical metrics), MC (all comparison metrics), WF (all efficiency metrics), and period trends (all yearly trend slopes). No filtering by threshold — the LLM decides what's notable.
...
<task type="auto">
  ...
  **Observation extraction logic `extractObservations()` — EXHAUSTIVE, no threshold filtering:**

  Include ALL metric comparisons from every available signal. The LLM decides what's notable, not the tool.

  - From rolling `recentVsHistorical.metrics`: map EVERY metric comparison to an observation (signal: 'rollingMetrics', current = recentValue, comparison = historicalValue)
  - From MC `comparison`: map EVERY MetricComparison to an observation (signal: 'regimeComparison', current = recentWindowValue, comparison = fullHistoryValue)
  - From WF `recentVsHistorical`: map EVERY efficiency metric (sharpe, winRate, profitFactor) to an observation (signal: 'walkForward', current = recent avg efficiency, comparison = historical avg efficiency)
  - From period `trends.yearly`: map EVERY metric's trend slope to an observation (signal: 'periodMetrics', current = slope, comparison = 0, delta = slope). Include all metrics that have trend data, not just those with negative slopes.
  - From live alignment (if available): include direction agreement rate (signal: 'liveAlignment', current = overallRate, comparison = 1.0) and execution efficiency (current = overallEfficiency, comparison = 1.0)

  Total: ~19-21 observations depending on available signals. ~3KB of JSON — trivial for LLM reasoning.
