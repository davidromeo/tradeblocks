...
must_haves:
  ...
  artifacts:
    - path: "packages/mcp-server/src/tools/edge-decay.ts"
      provides: "Fifth edge decay MCP tool: analyze_live_alignment"
...
<objective>
Register `analyze_live_alignment` as the fifth edge decay MCP tool, wiring the lib calculation engine to block/reporting log loading with graceful skip when no reporting log exists.

Purpose: Makes live alignment signals accessible to Claude and MCP clients, completing the four signal categories needed by Phase 50's verdict synthesis.
...
<task type="auto">
  <name>Task 1: Register analyze_live_alignment MCP tool</name>
  <files>
    packages/mcp-server/src/tools/edge-decay.ts
    packages/mcp-server/package.json
  </files>
  <action>
Add the fifth edge decay tool to `packages/mcp-server/src/tools/edge-decay.ts` following the exact pattern of Tools 1-4.
...
2. **Add imports:**
   - Add `analyzeLiveAlignment` to the `@tradeblocks/lib` import
   - Add `type { ReportingTrade }` to the `@tradeblocks/lib` import
   - Add `import { loadReportingLog } from "../utils/block-loader.js";`

3. **Update `filterByStrategy`** to also handle `ReportingTrade[]`:
   Create a second overload or make the existing function generic. The simplest approach: add a new function `filterReportingByStrategy` that filters `ReportingTrade[]` by strategy (case-insensitive). Or make `filterByStrategy` accept `Array<{ strategy: string }>` and return the same type using generics:
   ```typescript
   function filterByStrategy<T extends { strategy: string }>(trades: T[], strategy?: string): T[] {
     if (!strategy) return trades;
     return trades.filter(t => t.strategy.toLowerCase() === strategy.toLowerCase());
   }
   ```
   This is the cleanest approach and maintains backward compatibility.

4. **Register Tool 5** after the WFD tool registration:
   ```typescript
...
5. **Version bump:** In `packages/mcp-server/package.json`, bump version from `0.7.2` to `0.7.3`.
...
3. `TRADEBLOCKS_DATA_DIR=~/backtests tradeblocks-mcp --call analyze_live_alignment '{"blockId":"main-port-2026"}'` -- returns either alignment metrics or graceful skip (depending on whether reporting log exists)
  ...
  <done>
`analyze_live_alignment` is registered as the fifth edge decay MCP tool. When reporting log exists, returns direction agreement, execution efficiency, and alignment trends. When no reporting log exists, returns `{ available: false, reason: "no reporting log" }` via createToolOutput (not error). MCP server builds cleanly and all existing tests pass.
  </done>
</task>
