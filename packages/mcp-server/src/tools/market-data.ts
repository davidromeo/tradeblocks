...
/**
 * Helper to safely get a field from a typed object by key
 */
...
function applyMarketFilters(
  ...
  return data.filter((d) => {
    if (filters.minVix !== undefined && d.VIX_Close < filters.minVix) return false;
    if (filters.maxVix !== undefined && d.VIX_Close > filters.maxVix) return false;

    if (filters.volRegime && !filters.volRegime.includes(d.Vol_Regime)) return false;

    if (filters.gapDirection) {
      if (filters.gapDirection === "up" && d.Gap_Pct <= 0) return false;
      if (filters.gapDirection === "down" && d.Gap_Pct >= 0) return false;
    }

    if (filters.minGapPct !== undefined && Math.abs(d.Gap_Pct) < filters.minGapPct) return false;
    if (filters.maxGapPct !== undefined && Math.abs(d.Gap_Pct) > filters.maxGapPct) return false;

    if (filters.dayOfWeek && !filters.dayOfWeek.includes(d.Day_of_Week)) return false;

    if (filters.termStructure !== undefined && d.Term_Structure_State !== filters.termStructure) return false;

    if (filters.minTrendScore !== undefined && d.Trend_Score < filters.minTrendScore) return false;
    if (filters.maxTrendScore !== undefined && d.Trend_Score > filters.maxTrendScore) return false;

    if (filters.isOpex !== undefined) {
      const isOpexDay = d.Is_Opex === 1;
      if (filters.isOpex !== isOpexDay) return false;
    }

    if (filters.minRsi !== undefined && d.RSI_14 < filters.minRsi) return false;
    if (filters.maxRsi !== undefined && d.RSI_14 > filters.maxRsi) return false;
...
export function registerMarketDataTools(server: McpServer, baseDir: string): void {
  // ---------------------------------------------------------------------------
  // get_market_context - Query market conditions for date(s)
  // ---------------------------------------------------------------------------
  server.registerTool(
    "get_market_context",
    {
      description:
        "Query market conditions (VIX, term structure, regime, etc.) for a specific date or date range. " +
        "Supports filtering by various market metrics.",
      inputSchema: z.object({
        ...
        filters: z.object({
          minVix: z.number().optional().describe("VIX_Close >= value"),
          maxVix: z.number().optional().describe("VIX_Close <= value"),
          volRegime: z.array(z.number()).optional().describe("Vol_Regime in [1-6]"),
          gapDirection: z.enum(["up", "down", "any"]).optional().describe("Gap direction filter"),
          minGapPct: z.number().optional().describe("|Gap_Pct| >= value"),
          maxGapPct: z.number().optional().describe("|Gap_Pct| <= value"),
          dayOfWeek: z.array(z.number()).optional().describe("Day of week (2=Mon...6=Fri)"),
          termStructure: z.number().optional().describe("-1=backwardation, 0=flat, 1=contango"),
          minTrendScore: z.number().optional().describe("Trend_Score >= value"),
          maxTrendScore: z.number().optional().describe("Trend_Score <= value"),
          isOpex: z.boolean().optional().describe("Filter for/against OPEX days"),
          minRsi: z.number().optional().describe("RSI_14 >= value"),
          maxRsi: z.number().optional().describe("RSI_14 <= value"),
        }).optional().describe("Optional filters"),
        includeIntraday: z.boolean().optional().describe("Include 15-min intraday checkpoint data (default: false)"),
        includeHighLow: z.boolean().optional().describe("Include high/low timing data (default: false)"),
        includeVix: z.boolean().optional().describe("Include VIX intraday data (default: false)"),
        flatten: z.boolean().optional().describe("Flatten nested intraday/highlow/vix into main record (default: false). When true, fields like MOC_15min appear at top level instead of nested under 'intraday'"),
        limit: z.number().optional().describe("Max rows to return (default: 100)"),
      }),
    },
    async ({ startDate, endDate, fields, filters, includeIntraday, includeHighLow, includeVix, flatten = false, limit = 100 }) => {
      try {
        const { daily, intraday, highlow, vixIntraday } = await getMarketData(baseDir);
        ...
        results = applyMarketFilters(results, filters);
        ...
        const days = results.map((d) => {
          let record: Record<string, unknown> = { date: d.date };

          if (fields && fields.length > 0) {
            // Return only requested fields
            for (const field of fields) {
              if (field in d) {
                record[field] = getField(d, field);
              }
            }
          } else {
            // Return all fields
          ...
          // Add intraday if requested
          if (includeIntraday) {
            const intradayRecord = intraday.get(d.date);
            if (intradayRecord) {
              if (flatten) {
                // Flatten: merge intraday fields into main record (skip duplicate 'date')
                for (const [key, value] of Object.entries(intradayRecord)) {
                  if (key !== "date") {
                    record[key] = value;
                  }
                }
              } else {
                record.intraday = intradayRecord;
              }
            }
          }

          // Add highlow if requested
          if (includeHighLow) {
            const highlowRecord = highlow.get(d.date);
            if (highlowRecord) {
              if (flatten) {
                for (const [key, value] of Object.entries(highlowRecord)) {
                  if (key !== "date") {
                    record[key] = value;
                  }
                }
              } else {
                record.highlow = highlowRecord;
              }
            }
          }

          // Add VIX intraday if requested
          if (includeVix) {
            const vixRecord = vixIntraday.get(d.date);
            if (vixRecord) {
              if (flatten) {
                for (const [key, value] of Object.entries(vixRecord)) {
                  if (key !== "date") {
                    record[key] = value;
                  }
                }
              } else {
                record.vix = vixRecord;
              }
            }
          }
        ...
        const summary = `Market context: ${startDate} to ${end} | ${totalMatching} days matching, ${days.length} returned`;

        return createToolOutput(summary, {
          query: { startDate, endDate: end, filters },
          totalMatching,
  ...
  // ---------------------------------------------------------------------------
  // enrich_trades - Join market data to existing block trades
  // ---------------------------------------------------------------------------
  server.registerTool(
    "enrich_trades",
    {
      description:
        "Join market context data to an existing block's trades. Returns trades with market conditions " +
        "on the day they were opened, enabling regime-based analysis.",
      inputSchema: z.object({
        blockId: z.string().describe("Block ID to enrich"),
        strategy: z.string().optional().describe("Filter to specific strategy"),
        fields: z.array(z.string()).optional().describe("Specific market fields to add (default: key fields)"),
        includeIntraday: z.boolean().optional().describe("Include 15-min intraday checkpoint data (default: false)"),
        includeHighLow: z.boolean().optional().describe("Include high/low timing data (default: false)"),
        includeVix: z.boolean().optional().describe("Include VIX intraday data (default: false)"),
      }),
    },
    async ({ blockId, strategy, fields, includeIntraday, includeHighLow, includeVix }) => {
      try {
        ...
        const enrichFields = fields || defaultFields;
        ...
        const enrichedTrades = trades.map((trade) => {
          ...
          for (const field of enrichFields) {
            if (field in marketData) {
              market[field] = getField(marketData, field);
            }
          }

          if (includeIntraday) {
            const intradayData = intraday.get(tradeDate);
            if (intradayData) {
              market.intraday = intradayData;
            }
          }

          if (includeHighLow) {
            const highlowData = highlow.get(tradeDate);
            if (highlowData) {
              market.highlow = highlowData;
            }
          }

          if (includeVix) {
            const vixData = vixIntraday.get(tradeDate);
            if (vixData) {
              market.vix = vixData;
            }
          }
        ...
        const summary = `Enriched ${matched}/${trades.length} trades with market context | ${notMatched} not matched`;

        return createToolOutput(summary, {
          blockId,
          strategy: strategy || null,
          tradesEnriched: matched,
          tradesNotMatched: notMatched,
          fieldsIncluded: enrichFields,
  ...
  // ---------------------------------------------------------------------------
  // find_similar_days - Find historically similar market days
  // ---------------------------------------------------------------------------
  server.registerTool(
    "find_similar_days",
    {
      description:
        "Find historical trading days with similar market conditions to a reference date or specified criteria. " +
        "Useful for pattern recognition and scenario analysis.",
      inputSchema: z.object({
        referenceDate: z.string().optional().describe("Find days similar to this date (YYYY-MM-DD)"),
        conditions: z.object({
          vixRange: z.tuple([z.number(), z.number()]).optional().describe("[min, max] VIX range"),
          gapPctRange: z.tuple([z.number(), z.number()]).optional().describe("[min, max] gap % range"),
          termStructure: z.number().optional().describe("-1, 0, or 1"),
          volRegime: z.array(z.number()).optional().describe("Vol regime values to match"),
          trendScoreRange: z.tuple([z.number(), z.number()]).optional().describe("[min, max] trend score"),
          rsiRange: z.tuple([z.number(), z.number()]).optional().describe("[min, max] RSI range"),
        }).optional().describe("Conditions to match (used if no referenceDate)"),
        excludeDates: z.array(z.string()).optional().describe("Dates to exclude from results"),
        limit: z.number().optional().describe("Max results (default: 20)"),
      }),
    },
    async ({ referenceDate, conditions, excludeDates, limit = 20 }) => {
      try {
        const { daily } = await getMarketData(baseDir);
        ...
        if (referenceDate) {
          const refData = daily.get(referenceDate);
          if (!refData) {
            return {
              content: [{ type: "text", text: `Reference date ${referenceDate} not found in market data` }],
              isError: true,
            };
          }

          // Create ranges around reference values (+/- tolerance)
          refConditions = {
            vixRange: [refData.VIX_Close - 3, refData.VIX_Close + 3],
            termStructure: refData.Term_Structure_State,
            volRegime: [refData.Vol_Regime],
          };
        } else if (conditions) {
          refConditions = conditions;
        } else {
          return {
            content: [{ type: "text", text: "Must provide either referenceDate or conditions" }],
            isError: true,
          };
        }
        ...
        if (referenceDate) excludeSet.add(referenceDate);
        ...
        for (const [date, data] of daily) {
          if (excludeSet.has(date)) continue;

          let score = 0;
          let factors = 0;

          // VIX similarity
          if (refConditions.vixRange) {
            const [min, max] = refConditions.vixRange;
            if (data.VIX_Close >= min && data.VIX_Close <= max) {
              // Score based on how close to center of range
              const center = (min + max) / 2;
              const range = max - min;
              const distance = Math.abs(data.VIX_Close - center) / (range / 2);
              score += 1 - distance;
              factors++;
            } else {
          ...
          // Term structure match
          if (refConditions.termStructure !== undefined) {
            if (data.Term_Structure_State === refConditions.termStructure) {
              score += 1;
              factors++;
            } else {
              continue; // Hard filter
          ...
          // Vol regime match
          if (refConditions.volRegime) {
            if (refConditions.volRegime.includes(data.Vol_Regime)) {
              score += 1;
              factors++;
            } else {
              continue; // Hard filter
          ...
          // Gap % range (soft filter)
          if (refConditions.gapPctRange) {
            const [min, max] = refConditions.gapPctRange;
            if (data.Gap_Pct >= min && data.Gap_Pct <= max) {
              score += 0.5;
              factors += 0.5;
            }
          }

          // Trend score range (soft filter)
          if (refConditions.trendScoreRange) {
            const [min, max] = refConditions.trendScoreRange;
            if (data.Trend_Score >= min && data.Trend_Score <= max) {
              score += 0.5;
              factors += 0.5;
            }
          }

          // RSI range (soft filter)
          if (refConditions.rsiRange) {
            const [min, max] = refConditions.rsiRange;
            if (data.RSI_14 >= min && data.RSI_14 <= max) {
              score += 0.5;
              factors += 0.5;
            }
          }
        ...
        const similarDays = topMatches.map((m) => ({
          date: m.date,
          similarityScore: Math.round(m.similarityScore * 100) / 100,
          VIX_Close: m.data.VIX_Close,
          Vol_Regime: m.data.Vol_Regime,
          Term_Structure_State: m.data.Term_Structure_State,
          Gap_Pct: Math.round(m.data.Gap_Pct * 1000) / 1000,
          Intraday_Return_Pct: Math.round(m.data.Intraday_Return_Pct * 1000) / 1000,
          Trend_Score: m.data.Trend_Score,
          Day_of_Week: m.data.Day_of_Week,
        }));

        const summary = `Found ${similarDays.length} similar days${referenceDate ? ` to ${referenceDate}` : ""}`;

        return createToolOutput(summary, {
          reference: referenceDate ? { date: referenceDate, ...daily.get(referenceDate) } : { conditions: refConditions },
          totalMatching: matches.length,
          returned: similarDays.length,
          similarDays,
        });
      } catch (error) {
        return {
