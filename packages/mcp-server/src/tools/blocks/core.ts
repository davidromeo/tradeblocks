    ...
    "get_trades",
    {
      ...
      inputSchema: z.object({
        ...
        outcome: z
          ...
          .default("all")
          .describe(
            "Filter by trade outcome: 'all', 'winners' (P&L > 0), 'losers' (P&L < 0), 'breakeven' (P&L = 0)"
          ),
        sortBy: z
      ...
      try {
        ...
        trades = [...trades].sort((a, b) => {
          switch (sortBy) {
            ...
            case "ticker": {
              const aTicker = (a.customFields?.["ticker"] ??
                a.customFields?.["Ticker"] ??
                "") as string;
              const bTicker = (b.customFields?.["ticker"] ??
                b.customFields?.["Ticker"] ??
                "") as string;
