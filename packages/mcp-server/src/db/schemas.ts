...
export async function ensureTradeDataTable(conn: DuckDBConnection): Promise<void> {
  ...
  // 'csv' = imported from Option Omega CSV, 'backtest' = generated by backtesting engine
  ...
  // Migration: add dte column for backtest DTE-at-entry (Phase 80 gap closure)
  if (!(await hasColumn(conn, "trades", "trade_data", "dte"))) {
    await conn.run(`ALTER TABLE trades.trade_data ADD COLUMN dte INTEGER`);
  }
  // Migration: add entry_greeks_json column for backtest entry greeks (Phase 80 gap closure)
