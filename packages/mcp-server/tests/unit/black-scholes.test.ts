...
describe('bsPrice', () => {
  ...
  test('ATM call price is approximately 8.59', () => {
    const price = bsPrice('call', S, K, T, r, q, sigma);
    expect(price).toBeCloseTo(8.59, 0);
    expect(price).toBeGreaterThan(7.5);
    expect(price).toBeLessThan(10.0);
  });

  test('ATM put price is approximately 8.64 (put-call parity)', () => {
    const putPrice = bsPrice('put', S, K, T, r, q, sigma);
    expect(putPrice).toBeCloseTo(8.64, 0);
...
describe('solveIV', () => {
  ...
  test('converges to ~0.20 for ATM call priced at ~8.59', () => {
  ...
  test('returns null for deep OTM option with price ~0', () => {
    const iv = solveIV('call', 0.0001, 100, 200, 0.01, 0.045, 0.015);
    expect(iv).toBeNull();
  });
...
describe('bsDelta', () => {
  ...
  test('ATM put delta is approximately -0.50', () => {
    const delta = bsDelta('put', S, K, T, r, q, sigma);
    expect(delta).toBeLessThan(-0.40);
...
describe('computeLegGreeks', () => {
  test('returns GreeksResult with all fields for valid inputs', () => {
    const result: GreeksResult = computeLegGreeks(8.59, 100, 100, 365, 'C', 0.045, 0.015);
  ...
  test('converts DTE to years internally (dte/365)', () => {
    ...
    const result365 = computeLegGreeks(8.59, 100, 100, 365, 'C', 0.045, 0.015);
    // Should get similar IV for the same option with T=1.0
    expect(result365.iv).not.toBeNull();
    expect(result365.iv!).toBeCloseTo(0.20, 1);
  });
