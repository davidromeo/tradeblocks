...
export class TradeProcessor {
    ...
    try {
      ...
      for (let i = 0; i < parseResult.data.length; i++) {
        try {
          ...
          invalidTrades++;
          const errorMessage = `Trade conversion failed at row ${i + 2}: ${
  ...
  private validateRawTradeData(
    ...
    try {
      ...
      if (
        !normalizedRow["Strategy"] ||
        normalizedRow["Strategy"].trim() === ""
      ) {
  ...
  private convertToTrade(rawData: Record<string, string>): Trade {
    try {
      ...
      // Build trade object
      const rawPremiumString = (rawData["Premium"] || "")
        .replace(/[$,]/g, "")
        .trim();
      const premiumPrecision: Trade["premiumPrecision"] =
        rawPremiumString && !rawPremiumString.includes(".")
          ? "cents"
          : "dollars";
      ...
      const trade: Trade = {
        ...
        numContracts: Math.round(
          parseNumber(rawData["No. of Contracts"], "No. of Contracts")
        ),
        ...
        closingShortLongRatio: rawData["Closing Short/Long Ratio"]
          ? parseNumber(
              rawData["Closing Short/Long Ratio"],
        ...
        movement: rawData["Movement"]
          ? parseNumber(rawData["Movement"], "Movement")
          : undefined,
        maxProfit: rawData["Max Profit"]
          ? parseNumber(rawData["Max Profit"], "Max Profit")
          : undefined,
        maxLoss: rawData["Max Loss"]
          ? parseNumber(rawData["Max Loss"], "Max Loss")
          : undefined,
      };
